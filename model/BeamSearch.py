...
def beam_search(model, smiVoc, num_beams, batch_size, max_length, topk, example, prop=None, device='cuda'):
    ...
    input_ids = torch.full((batch_size * num_beams, 1), sos_token_id, dtype=torch.long, device=device)

    enc_outputs1, enc_pad_mask1, msa_outputs = model.model.encoder(node_attr, pos, batch, atom_laplacian)
    # enc_outputs2, enc_pad_mask2 = model.encoder2(
    #     aa_node_attr,
    #     aa_pos,
    #     aa_batch,
    #     aa_laplacian,
    ...
    pad_attn_mask = enc_pad_mask1

    enc_outputs = enc_outputs.repeat_interleave(num_beams, 0)
    pad_attn_mask = pad_attn_mask.repeat_interleave(num_beams, 0)

    while cur_len < max_length:
        dec_outputs = model.model.decoder(input_ids, enc_outputs, pad_attn_mask, cur_len, prop)
        dec_logits = model.model.projection(dec_outputs)
